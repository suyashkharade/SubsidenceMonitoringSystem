...
public class Map1 extends Mapper<LongWritable,Text,LongWritable,DoubleWritable> {
	...
	//static int width = 4894;//s 20 20 is border
	//static int height =5158;// 1738
	...
	public void map(LongWritable key,Text value,
 	   ...
 	   if(linecount==0)
 	   {
 		  ...
 		  keys[pixelcount]=((Long.parseLong(st.nextToken())/multilookL)*(width/multilookP)*2);
 		  keys[pixelcount]+=(Long.parseLong(st.nextToken())/multilookP)*2;
 		  RLINE[pixelcount]=Double.parseDouble(st.nextToken());
...
/////////////////////////////
 	  if(Ilinecount==0)
	   {
		  ...
		  ILINE[Ipixelcount]=-Double.parseDouble(st.nextToken());//see negative sign here
