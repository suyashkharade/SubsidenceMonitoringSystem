...
public class Reduce extends  Reducer<LongWritable,IntWritable,Text,Text> {
...
*/
    	     last_real=values.iterator().next().get();
    	 //key.set(key.get()/2);
     ...
     else
     {
    	 ...
    	 key.set(key.get()/2);
    	 txtkey.set(""+key.get()/width+"\t"+key.get()%width);
    	 txtvalue.set("\t"+last_real+"\t"+values.iterator().next().get());
    context.write(txtkey, txtvalue);
    	 //	 multipleOutputs.write(txtkey,txtvalue,"/app/hadoop/tmp/slc1/Phase_ersDH012");
     }		
