...
public class NLineInputFormat extends FileInputFormat<LongWritable, Text> {
  ...
  public static final String OVERLAP_LINES=  "mapreduce.input.lineinputformat.overlaplines";
	...
	public List<InputSplit> getSplits(JobContext job) throws IOException {
		...
		int overlapLines = getOverlapLinesPerSplit(job);//ara added
		for (FileStatus status : listStatus(job)) {
			splits.addAll(getSplitsForFile(status, job.getConfiguration(),
					numLinesPerSplit,overlapLines));
		}
		return splits;
	}

	public static List<FileSplit> getSplitsForFile(FileStatus status,
			Configuration conf, int numLinesPerSplit,int overlapLines) throws IOException {
		...
		try {
			...
			long begin = 0;
			long length = 0;
			long length_before_overlap=0;
			int num = -1;
			while ((num = lr.readLine(line)) > 0) {
				numLines++;
				length += num;
				if(numLines==(numLinesPerSplit-overlapLines))
		              length_before_overlap=length;
				if (numLines == numLinesPerSplit) {
					...
					begin += length_before_overlap; //logic  to send overlap
					length -=length_before_overlap; //logic to send overlap 
					numLines = (overlapLines);
				}
			}
			if (numLines != 0) {
	...
	public static void setOverlapLinesPerSplit(Job job, int overlapLines) {
		job.getConfiguration().setInt(OVERLAP_LINES, overlapLines);
	}
