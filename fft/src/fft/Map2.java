...
*/
	...
	public void map(LongWritable key, Text value,
		...
		for(int i=0;i<64;i++)
		{
			for(int j=0;j<64;j++)
			{
			context.write(new LongWritable(4*keys[cnt]+2), new DoubleWritable(mat[i][j]));
			avg1=avg1+mat[i][j];
			context.write(new LongWritable(4*keys[cnt]+3), new DoubleWritable(mati[i][j]));
			avg2=avg2+mati[i][j];
			cnt++;
			}
		}
		context.write(new LongWritable(-(4*(keys[cnt-1]/4096)+3)), new DoubleWritable(avg1/4096));
		context.write(new LongWritable(-(4*(keys[cnt-1]/4096)+4)), new DoubleWritable(avg2/4096));
		/*
